...
const MODES: { value: SeismicMode; label: string }[] = [
  { value: "precomputed", label: "Precomputed" },
  { value: "train", label: "Train GNN" },
];
...
const DISPLAYS: { value: TrainingDisplay; label: string }[] = [
  { value: "both", label: "Both" },
  { value: "nodes", label: "Connectivity" },
  { value: "links", label: "Link failure" },
];
...
export interface TrainingControls {
  ...
  onDisplayChange: (display: TrainingDisplay) => void;
  onTrain: () => void;
  onReset: () => void;
}
...
interface SidebarProps {
  ...
  loading: boolean;
  error: string | null;
  onModeChange: (mode: SeismicMode) => void;
...
function LossChart({ train, test }: { train: number[]; test: number[] }) {
  ...
  const toPoints = (values: number[]) =>
    values
      .map((v, i) => `${(i / Math.max(values.length - 1, 1)) * 100},${30 - (clamp(v) / scale) * 28}`)
      .join(" ");
  return (
...
function TrainingPanel({ selectedNode, severity, display, job, trainError, onSeverityChange, onDisplayChange, onTrain, onReset }: TrainingControls) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <p className="text-[11px] leading-snug text-slate-500">
        Click a node on the map to set it as the destination, choose an earthquake scenario, then train a GraphSAGE surrogate on 100 simulated earthquakes.
      </p>

      <div className="flex items-center justify-between">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Destination</span>
        <span className="text-xs font-medium tabular-nums text-slate-600">
          {selectedNode === null ? "click a map node" : `Node ${selectedNode}`}
        </span>
      </div>

      <div className="flex flex-col gap-1">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Earthquake scenario</span>
      ...
      {completed && job?.result && (
        <div className="flex flex-col gap-2">
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs text-slate-400">MAE on {job.earthquake_type} scenario · {job.size} network</span>
          </div>
          <p className="text-[11px] leading-snug text-slate-500">
            Click nodes for predicted vs simulated connectivity to Node {job.target_node_id}, and links for their failure probability.
          </p>

          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Show on map</span>
            <div className="flex gap-1 rounded-xl bg-slate-100 p-1">
              {DISPLAYS.map((option) => (
                <button
                  key={option.value}
                  onClick={() => onDisplayChange(option.value)}
                  className={`flex-1 rounded-lg px-1 py-1 text-[10.5px] font-medium transition-colors ${display === option.value ? "bg-white text-teal-700 shadow-[0_1px_2px_rgba(15,23,42,0.1)]" : "text-slate-500 hover:text-slate-700"}`}
                >
                  {option.label}
                </button>
              ))}
          ...
          {display !== "links" && (
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-medium text-slate-500">Node connectivity (predicted)</span>
          ...
          {display !== "nodes" && (
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-medium text-slate-500">Link failure probability</span>
              <div className="h-2 w-full rounded-full" style={{ background: "linear-gradient(to right, #0d9488, #f59e0b 50%, #dc2626)" }} />
              <div className="flex justify-between text-[10px] font-medium text-slate-500">
                <span>Unlikely to fail</span>
...
export default function Sidebar({ mode, size, graph, loading, error, onModeChange, onSizeChange, training }: SidebarProps) {
  ...
  return (
    <aside className="flex max-h-full w-full flex-col overflow-hidden rounded-2xl border border-slate-900/[0.07] bg-white text-slate-800 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_30px_rgba(15,23,42,0.08)]">
        ...
        <div className="flex gap-1 rounded-xl bg-slate-100 p-1">
          {MODES.map((option) => (
            <button
              ...
              className={`flex-1 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${mode === option.value ? "bg-white text-teal-700 shadow-[0_1px_2px_rgba(15,23,42,0.1)]" : "text-slate-500 hover:text-slate-700"}`}
            >
              {option.label}
            </button>
          ))}
        ...
        <div className="flex gap-1 rounded-xl bg-slate-100 p-1">
          {SIZES.map((option) => (
            <button
              ...
              className={`flex-1 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors disabled:opacity-50 ${size === option.value ? "bg-white text-teal-700 shadow-[0_1px_2px_rgba(15,23,42,0.1)]" : "text-slate-500 hover:text-slate-700"}`}
            >
              {option.label}
            </button>
          ))}
      ...
      <div className="mb-4 flex min-h-0 flex-col gap-4 overflow-y-auto px-4">
              ...
              <>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="mt-1.5 flex items-center gap-2">
                    <span className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full border-[3px] border-indigo-600 bg-white">
        ...
        {mode === "train" && <TrainingPanel {...training} />}
      </div>
    </aside>
  );
}
