...
function StudyCard({ study }: { study: Study }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-5 border-t border-slate-900/[0.05] pt-4 text-xs leading-relaxed text-slate-400">
        {study.paper}
      </p>
      ...
      <span className={`mt-4 inline-flex items-center gap-1.5 text-sm font-medium ${available ? "text-teal-700" : "text-slate-400"}`}>
        ...
        {available && (
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="transition-transform duration-200 ease-out group-hover:translate-x-0.5" aria-hidden="true">
    ...
    <main className="min-h-screen w-full bg-slate-50 text-slate-800">
      <div className="mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center px-6 py-16">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-semibold tracking-tight text-slate-900">Resilient Routes</span>
        </div>

        <h1 className="mt-8 text-3xl font-semibold leading-tight tracking-tight text-slate-900 text-balance sm:text-4xl">
          Graph neural network surrogates for transportation resilience
        </h1>
