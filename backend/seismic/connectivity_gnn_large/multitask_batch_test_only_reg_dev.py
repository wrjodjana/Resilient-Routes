...
from archived.v1.dataset.connectivity_gnn_large.utils_v4_dev import *
from archived.v1.dataset.connectivity_gnn_large.modelzoo_v4_rev import *
...
class BridgeDataset(DGLDataset):
    def __init__(self, start_id, end_id, data_dir):
        ...
        self.end_id = end_id
        self.data_dir = os.path.join("connectivity_gnn_large","data", data_dir) 
...
checkpoint = torch.load('./connectivity_gnn_large/saved_model_dev/{}_{}_{}_{}.pth'.format(model_idx, batch_size, dataset_name, percentage), map_location=device)
model.load_state_dict(checkpoint['model_state_dict'])

result_summary = []
reg_pred_summary = []

val_dir = 'data_{}_v4'.format(target_node_id)    # path to load different data
img_dir = 'img_reg_{}_v4'.format(target_node_id)  # path to save the prediction
