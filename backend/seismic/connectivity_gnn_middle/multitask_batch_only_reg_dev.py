...
from archived.v1.dataset.connectivity_gnn_middle.utils_v3_dev import load_edge_list, generate_sample, cal_label_distribution
from archived.v1.dataset.connectivity_gnn_middle.modelzoo_v3_rev import *
...
def train_multitask(trainloader, testloader, percentage, model, \
    ...
    if plot_history:
        ...
        plt.ylabel('L1 error')
        plt.savefig('./img/img_meeting_v3/loss_2.jpg')
        plt.close('all')
    if save_model:
...
if val_different:
    ...
    target_node_id = 19
    val_num_sample = [100]
    val_data_dir = ['./{}/data_{}_v3'.format(dataset_name, target_node_id)]    # path to load different data
    val_img_dir = ['./{}/img_reg_{}_v3'.format(imageset_name, target_node_id)]      # path to save the prediction
