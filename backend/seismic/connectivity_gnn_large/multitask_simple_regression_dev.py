...
from archived.v1.dataset.connectivity_gnn_large.utils_v4_dev import load_edge_list, generate_sample, reserve_sample
    ...
    def process(self):
        ...
        for num_sample, dir in zip(num_sample_list, data_dir_list):
            ...
            for graph_id in graph_id_range:
                ...
                # feat = np.concatenate((graph_data['edge_feat'].reshape(-1), graph_data['node_feat'].reshape(-1)))
...
### path to load training data
test_data_dir = ['./{}/data_{}_v4'.format(dataset_name, target_id) for target_id in test_targets]
